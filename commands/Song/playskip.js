  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (!args[0]) {
      ...
      if(message.content.includes("youtu")){
        //send searching
        message.channel.send(`<:818555971873013761:935061514161889282> **Searching** :mag_right: \`${args.join(" ")}\``)
      ...
      //IF SPOTIFY SEARCH SEND INFO WITH SPOTIFY
        ...
        //send searching
        message.channel.send(`<:846090652231663647:965257997972545617> **Searching** :mag_right: \`${args.join(" ")}\``)
        //play the song from our playermanager
        playermanager(client, message, args, `play:youtube`);
      //IF SOUNDCLOUD SEARCH SEND INFO WITH SOUNDCLOUD
      } else if(message.content.includes("soundcloud")){
        //send searching
        message.channel.send(`<:images:1032607542079127583> **Searching** :mag_right: \`${args.join(" ")}\``)
        //play the song from our playermanager
        playermanager(client, message, args, `play:soundcloud`);
      //ELSE SEND RYTHM INFO
    } else if(message.content.includes("http")){
      //send searching
      message.channel.send(`<:rythm:1051414768503758858> **Searching** :mag_right: \`${args.join(" ")}\``)
      //play the song from our playermanager
      playermanager(client, message, args, `play:youtube`);
    } else {
      //send searching
